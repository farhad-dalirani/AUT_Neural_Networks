...
class MLP:
    ...
    def fit_momentum_stocastic(self, X, y):
        ...
        for epoch in range(0, self.__max_epoch):
            ...
            if epoch % 100 == 0 or epoch == self.__max_epoch-1:
                print('\rTRAINING, epoch {}/{}, cost: {}'.format(epoch, self.__max_epoch, cost))
...
if __name__ == '__main__':
    ...
    mlp = MLP(n=n, activations=activations, type_of_cost='MSE', learning_rate=0.01, max_epoch=200, mode='stochastic', random_state=0)


    mlp.fit(X=trainX, y=trainY)

    catch_I, catch_Y = mlp.forward_propagation(trainX)
    mse_score = mse(target=trainY, y=catch_Y[-1])
    print('\nMSE of training: {}'.format(mse_score))
    #print(trainY.shape)
    print(catch_Y[-1].shape)
