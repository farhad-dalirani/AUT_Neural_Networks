"""
This python code use 'perceptron_one_vs_all' class for classfying iris dataset.
...
if __name__ == '__main__':
    ...
    if key_file == '1':
        ...
        # determine user wants train perceptrons or read trained perceptrons from file
        train_yes_no = int(input('Do you want to train Perceptrons(one vs all) or read trained perceptrons(weights) from file?'+ '\n'+
          '>    1: I want to give parameters and train Perceptrons'+ '\n'+
          '>    2: Read trained perceptrons from file'+ '\n'+
          'Input 1 or 2: '))
        ...
        if train_yes_no == '1':
            ...
            order = str(input('\nDo you want to use first order or second order perceptrons ?' + '\n'
            ...
            cut_error = float(input('\n> Do you want perceptrons stop training when error during training falls under a threshold:'+'\n'+
            ...
            do_train = False
            # read necessary input and parameters from standard input
            order = str(input('\n >Trained perceptrons in file are first order or second order?' + '\n'
                              + '    Enter 1 for first order ' + '\n' +
                              '   Enter 2 for second order'+'\n'+'Enter 1 or 2:'))
            ...
            random_state = int(
                float(input('\n> Enter an integer, it is used as seed for generating random numbers and' + '\n'
                            + ' repetitain of experiment:')))
            # name of jason file which trained perceptrons are there
            file_name = input(
    ...
    if do_train == True:
        units = perceptron_one_vs_all(number_classes=3, learning_rate=learning_rate, max_epoch=max_epochs, cut_error=cut_error)
        k_fold_units = perceptron_one_vs_all(number_classes=3, learning_rate=learning_rate, max_epoch=max_epochs, cut_error=cut_error)
    else:
        units = perceptron_one_vs_all(number_classes=3)
        k_fold_units = perceptron_one_vs_all(number_classes=3)
    ...
    if do_train == True:
        ...
        units.load_perceptorns_from_file(file_name)
