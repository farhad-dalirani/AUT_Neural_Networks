"""This file contains a class for multi-class perceptron unit"""
import numpy as np
...
class multi_class_perceptron:
    """
    Attributes:
        __number_of_classes: number of different classes
        __learning_rate: learning rate of multi-class Perceptron
        __weights: weights of multi-class perceptron
        __maximum_epoch: maximum number of epochs that multi-class perceptron is
        ...
        __weights_epochs: weights of multi-class perceptron in each epoch
        __cut_error: stop training if error fall under specific value
    """

    def __init__(self, number_of_classes, learning_rate=0.01, max_epoch=100, cut_error=None):
        """
        This function is constructor of multi-class Perceptron which sets
        different parameter of multi-class Perceptron.
        :param number_of_classes: number of different labels
        :param learning_rate: learning rate of multi-class Perceptron for updating weights
        :param max_epoch: maximum number of iterations that multi-class perceptron is
                        allowed to use for training
    ...
    def net_input(self, X):
        """
        Net input of multi-class perceptron for instance X, which equal: transpose(W)*X+W[0]
        :param X: Input instance
        :return: net input of multi-class perceptron
        """
        #print('X: ', X)
    ...
    def activation(self, net_inp):
        ...
        :param net_inp: net input of multi-class perceptron
        ...
        dis_to_class = []
        for classi in range(1, self.__number_of_classes+1):
            dis_to_class.append(abs(net_inp - classi))

        out = np.argmin(dis_to_class) + 1

        return out

    def predict(self, X):
        """
        This function predict output of multi-class perceptron for instance(s) X
    ...
    def fit(self, X, y, validX=None, validY=None, class_name="", rand_state=0, plotting=False, write_in_file=False):
        """
        This method of multi-class perceptron class gets training dataset and
        its label, then it trains multi-class perceptron. after doing "__max_epoch" epochs,
        ...
        # for maximum number of epochs train multi-class perceptron by training set
        for epoch in range(0, self.__max_epoch):

            # fit multi-class perceptron by giving all instances of training set
            # one by one
            error = 0
            for x, target in zip(X, y):

                # output of multi-class perceptron for sample x
        ...
        if write_in_file == True:
            with open('output-multi-class-peceptron.json', 'w') as outputfile:
    ...
    def load_weights(self, weights):
        """
        Load weights of multi-class perceptron from file
    ...
    def load_multi_class_perceptron_from_file(self, file_name):
        '''
...
#                  multi-class Perceptron class                 #
#                                                   #
#####################################################
if __name__ == '__main__':

    # small example for testing multi-class perceptron class
    ...
    # read train and test from iris dataset
    trainX, validX, testX, trainY, validY, testY = read_iris_dataset_FLNN(rand_state=0,
                                                                     train_ratio=0.60,
                                                                     valid_ratio=0.20,
                                                                     test_ratio=0.20)
    ...
    # create and train multi-class perceptron
    unit = multi_class_perceptron(number_of_classes=3, learning_rate=0.001, max_epoch=400)
